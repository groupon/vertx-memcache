...
/**
 * Tests for <code>KetamaContinuum</code>.
 *
 * @author Stuart Siegrist (fsiegrist at groupon dot com)
 * @since 1.0.0
 */
public class KetamaContinuumTest {
    ...
    public void testDefaultConstructor() {
        KetamaContinuum continuum = new KetamaContinuum(Arrays.asList(server1, server2, server3),
                HashAlgorithm.FNV1_32_HASH, 1);

        TreeMap<Long, MemcacheServer> map = continuum.getServerContinuum();
        assertEquals("Unexpected server count", 12, map.size());
    }
